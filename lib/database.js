...
module.exports = {
  ...
  get_stats: function(coin, cb) {
    Stats.findOne({coin: coin}, function(err, stats) {
      if(stats) {
        find_address(settings.burnAddress, function(address) {
          if(!isNaN(address.balance)){
            stats.supply = stats.supply - (address.balance/100000000);
            stats.burnt = (address.balance/100000000);
          }
