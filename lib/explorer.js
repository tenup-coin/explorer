...
module.exports = {
  ...
  get_hashrate: function(cb) {
    ...
    if (settings.nethash == 'netmhashps') {
      ...
      request({uri: uri, json: true}, function (error, response, result) {
        var body = result.result;
        if (result.error || body == 'There was an error. Check your console.') {
          return cb('-');
        } else {
          if (settings.nethash_units == 'K') {
  ...
  get_burntAccount: function(cb) {
    if ( settings.burnAddress && settings.burnAddress != "" ) {
      Address.findOne({a_id: settings.burnAddress}, function(err, address) {
        if (address) {
          return cb(address);
        } else {
          return cb();
  ...
  get_supply: function(cb) {
    if (settings.baseType === 'ripple') {
      ...
      request({uri: uri, json: true}, function (error, response, body) {
        const _body = body;
	console.log("OSupply " + body.moneysupply);
        module.exports.get_burntAccount(function(burnAccount){
          console.log("Supply: " + _body.moneysupply + ", Burnt: " + burnAccount.balance);
          return cb(_body.moneysupply);
        });
      });
      ...
      request({uri: uri, json: true}, function (error, response, body) {
        return cb(body.result.total_amount);
      });
    } else {
